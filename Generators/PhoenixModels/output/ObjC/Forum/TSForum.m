...
- (NSString *)dbKey {
    ...
    return [[self class] dbKeyWithID:self.forumID];
	
}

+ (NSString *)dbKeyWithID: (NSNumber *)identifier {
    return [NSString stringWithFormat:@"Phoenix/Forum/%@", [identifier stringValue]];
}
...
+ (NSString *)dbCollection {
  ...
  return @"Phoenix/Forum";
  
}

+ (instancetype)forumWithID: (NSNumber *)objectID {
